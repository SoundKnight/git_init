...
def connect_ftp(
    ...
    for dir_info in l_dir:
        ...
        # Falta mas codigo para que descargue los archivos dentro de las carpetas
        # for file_lib in l_subdir:
            # lib_name = get_file_name(file_lib)
            # print(lib_name)
            # save_file(connection, f"{actual_path}/{lib_name}", f"{save_path}/{lib_name}")
        print(l_subdir)
        # save_file(connection, f"{actual_path}/{file_name}", f"{save_path}/{file_name}")
...
if __name__ == "__main__":
    ...
    connect_ftp(host="ftp.mirror.nl", save_path="./TXT")
    # connect_ftp(host="ftp.heanet.ie", save_path="./TXT")
